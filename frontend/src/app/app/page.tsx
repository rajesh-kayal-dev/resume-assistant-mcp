          ...
          <div className="p-6 flex-1 overflow-y-auto space-y-6">
            ...
            <div className="border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#03050a] shadow-sm dark:shadow-none">
              ...
              <textarea
                ...
                className="w-full bg-white dark:bg-[#04060a] p-4 text-xs font-mono text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-700 outline-none resize-none h-24"
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto p-6 md:p-10">
                <div className="max-w-4xl mx-auto space-y-8 pb-20">
                  ...
                  {activeTab === "ats" && (
                    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                      <div className="border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#03050a] p-8 flex flex-col md:flex-row items-center gap-10 shadow-sm dark:shadow-none">
                        <ProgressRing value={results.atsScore.overall} />
                        <div className="flex-1">
                          ...
                          <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed max-w-lg mb-6">
                            Based on our semantic analysis, your resume matches {results.atsScore.overall}% of the target job requirements.
                          </p>
                          <div className="grid grid-cols-3 gap-4">
                            {["Keywords", "Formatting", "Impact"].map((cat, i) => (
                              <div key={cat} className="border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-[#04060a] p-4 text-center">
                                <div className="font-mono text-xl text-slate-900 dark:text-white">{[results.atsScore.keywords, results.atsScore.formatting, results.atsScore.impact][i]}%</div>
                      ...
                      {results.improvements.map((imp: any, i: number) => (
                        <div key={i} className="border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#03050a] p-6 hover:border-slate-300 dark:hover:border-cyan-900 transition-colors shadow-sm dark:shadow-none">
                          <div className="flex items-start gap-4">
                            ...
                            <div>
                              <div className="font-mono text-[10px] text-slate-500 dark:text-cyan-500 mb-1 tracking-widest uppercase">{imp.category}</div>
                              <h3 className="text-slate-900 dark:text-white font-medium mb-2">{imp.suggestion}</h3>
                              <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">{imp.reasoning}</p>
                              <div className="mt-4 p-4 bg-slate-50 dark:bg-[#04060a] border border-slate-200 dark:border-slate-800 font-mono text-xs text-slate-700 dark:text-slate-300">
                                <div className="text-slate-500 mb-2">// Suggested fix:</div>
                                {imp.example}
                              </div>
                            </div>
                          </div>
                  ...
                  {activeTab === "interview" && (
                    <div className="space-y-6 animate-in fade-in duration-500">
                      {results.interviewQuestions.map((q: any, i: number) => (
                        <div key={i} className="border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#03050a] p-6 hover:border-slate-300 dark:hover:border-cyan-900 shadow-sm dark:shadow-none">
                          ...
                          <p className="text-slate-600 dark:text-slate-400 text-sm mb-4 leading-relaxed">{q.why}</p>
                          <div className="p-4 bg-slate-50 dark:bg-[#04060a] border border-slate-200 dark:border-slate-800 border-l-2 border-l-slate-800 dark:border-l-cyan-500">
                            <div className="font-mono text-[10px] text-slate-500 dark:text-cyan-500 mb-2">IDEAL RESPONSE FRAMEWORK:</div>
                            <p className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed">{q.howToAnswer}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  {activeTab === "skills" && (
                    <div className="border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#03050a] p-8 animate-in fade-in duration-500 shadow-sm dark:shadow-none">
                      <div className="relative pl-8">
                        ...
                        {results.skillGaps.map((sg: any, i: number) => (
                          <div key={i} className="relative pb-10 last:pb-0">
                            ...
                            <h3 className="text-lg font-medium text-slate-900 dark:text-white mb-2">{sg.skill}</h3>
                            <div className="inline-flex px-2 py-1 bg-amber-50 dark:bg-slate-900 border border-amber-200 dark:border-slate-800 font-mono text-[10px] text-amber-700 dark:text-amber-400 mb-4">
                              EST. TIME: {sg.estimatedTimeToLearn}
                            </div>
                            <div className="space-y-2 mt-2">
                              {sg.resources.map((res: string, j: number) => (
                                <div key={j} className="flex items-center gap-3 text-sm text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-[#04060a] p-3 border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700">
                                  <ChevronRight size={14} className="text-slate-400 dark:text-cyan-500" />
                                  {res}
                                </div>
                              ))}
                            </div>
                  ...
                  {activeTab === "linkedin" && (
                    <div className="space-y-6 animate-in fade-in duration-500">
                      {!linkedinText ? (
                        ...
                        results.linkedinOptimization.map((opt: any, i: number) => (
                          <div key={i} className="border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#03050a] p-6 hover:border-slate-300 dark:hover:border-cyan-900 shadow-sm dark:shadow-none">
                            <div className="font-mono text-[10px] tracking-widest text-slate-500 dark:text-cyan-500 mb-4 uppercase">{opt.section}</div>
                            <p className="text-slate-700 dark:text-slate-300 text-sm mb-4 leading-relaxed">{opt.advice}</p>
                            <div className="p-4 bg-slate-50 dark:bg-[#04060a] border border-slate-200 dark:border-slate-800 font-mono text-xs text-emerald-700 dark:text-emerald-400">
                              <div className="text-slate-500 mb-2">// Suggested change:</div>
                              {opt.suggestedText}
                            </div>
                          </div>
                        ))
